...
export default function Signup() {
  ...
  const { login } = useUser();
  const { data, registerStudent } = useSchoolStore();
  ...
  const handleSignup = async () => {
    const { name, email, password, classLevel, board } = form;
    ...
    <View style={{ flex: 1, backgroundColor: '#1A237E' }}>
      <ScrollView contentContainerStyle={{
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 24 }}>
          ...
          <View>
            <Text style={{ fontSize: 26, fontWeight: 'bold', color: '#FFFFFF' }}>Student Registration</Text>
        ...
        <View style={{ backgroundColor: '#FFFFFF', borderRadius: 20, padding: 24 }}>
          ...
          <Field label="Full Name *" placeholder="Ahmed Ali" value={form.name} onChangeText={(v) => update('name', v)} />
          <Field label="Email *" placeholder="ahmed@email.com" value={form.email} onChangeText={(v) => update('email', v)} keyboardType="email-address" />
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 14 }}>
            <View style={{ flexDirection: 'row', gap: 8 }}>
              {CLASSES.map((c) => (
                <TouchableOpacity
                  key={c}
                  onPress={() => update('classLevel', c)}
                  style={{
                    width: 44, height: 44, borderRadius: 22,
                    backgroundColor: form.classLevel === c ? '#1A237E' : '#F5F5F5',
                    alignItems: 'center', justifyContent: 'center',
                    borderWidth: 1, borderColor: form.classLevel === c ? '#1A237E' : '#E0E0E0',
                  }}
                >
                  <Text style={{
                    color: form.classLevel === c ? '#FFF' : '#424242',
            ...
            {BOARDS.map((b) => (
              <TouchableOpacity
                ...
                style={{
                  ...
                  backgroundColor: form.board === b ? '#1A237E' : '#F5F5F5',
                  borderWidth: 1, borderColor: form.board === b ? '#1A237E' : '#E0E0E0',
                }}
              >
                <Text style={{
                  color: form.board === b ? '#FFF' : '#424242',
                  fontWeight: form.board === b ? '600' : '400', fontSize: 13,
                }}>{b}</Text>
              </TouchableOpacity>
            ))}
          </View>
          ...
          <TouchableOpacity
            ...
            disabled={isLoading || !foundSchool}
            style={{
              backgroundColor: isLoading || !foundSchool ? '#9FA8DA' : '#1A237E',
